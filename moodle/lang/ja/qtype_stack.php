...
/**
 * Strings for component 'qtype_stack', language 'ja', version '3.9'.
 *
 * @package     qtype_stack
 * @category    string
...
$string['deploy'] = 'デプロイ';
$string['deployduplicateerror'] = 'デプロイされた問題ケースで重複した問題メモが検出されました。各問題メモは一度だけデプロイすることを強くお勧めします。そうでない場合，問題ケースでグループ化するときに意味のある統計情報を集めるのが難しくなります。 重複するノートを持ついくつかの問題ケースを削除することを検討してください。';
...
$string['deploymanynotes'] = '新しい問題記録の作成に10度失敗するか，問題テストに1度失敗するとSTACKはそれらを中止します。';
...
$string['editquestioninthequestionbank'] = '問題の編集';
...
$string['healthchecksamplecas'] = '{@ x^4/(1+x^4) @} の微分は \\[ \\frac{d}{dx} \\frac{x^4}{1+x^4} = {@ diff(x^4/(1+x^4),x) @}. \\] Unicodeがサポートされているかどうかを確認します：\\(\\ forall \\)は{@unicode(8704)@}と表示されます。';
...
$string['runquestiontests_alert'] = '問題にはテストかケースが不足しています。';
$string['runquestiontests_help'] = '問題テストでは，動作が教師の意図と一致することを確認します。デプロイされた問題により，学生に提示されるランダム化された問題をあらかじめ確認することができます。これらは，信頼できる質問を作成してテストするのに役立つツールであり，問題が学生によって使用される全ての場合に使用する必要があります。';
...
$string['settingmaximalibraries_desc'] = 'これはカンマで区切られたMaximaのライブラリ名のリストで，Maximaに自動的にロードされます。サポートされているライブラリのみ使用することができます： "stats、distrib、descriptive、simplex"。もしパフォーマンスの最適化のためにMaximaイメージが保存されている場合は，ライブラリは読み込まれません。';
...
$string['stackversionerror'] = 'この問題は{$a->qfield}で{$a->pat}を使っていますが，STACK{$a->ver}以降ではサポートされていません。';
...
$string['tidyquestion'] = 'STACKの問題の整頓';
